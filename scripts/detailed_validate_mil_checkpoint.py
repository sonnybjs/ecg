...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Detailed CSV-level validation for one MIL checkpoint.")
...
def next_mil_eval_dir() -> Path:
    root = OUTPUT_ROOT / "mil_evaluations"
    root.mkdir(parents=True, exist_ok=True)
    existing = sorted(p for p in root.glob("detailed_mil_eval_*") if p.is_dir())
    next_idx = max((int(p.name.split("_")[-1]) for p in existing), default=0) + 1
    path = root / f"detailed_mil_eval_{next_idx:03d}"
...
def build_eval_config(args: argparse.Namespace, ckpt: dict, mapping) -> ExperimentConfig:
    ...
    if max_bags is None:
        max_bags = checkpoint_config_value(ckpt, "validation_max_batches" if args.split == "val" else "test_max_batches", 760)
    ...
    return ExperimentConfig(
        ...
        validation_max_batches=max_bags if args.split == "val" else 0,
        test_max_batches=max_bags if args.split == "test" else 0,
...
def main() -> None:
    ...
    logger.log(
        f"[{args.split}] using class-balanced subset bags={len(eval_records)}/{len(split_records)} "
        f"max_bags={max_bags}"
    )
    ...
    with torch.no_grad():
        for batch_idx, (bags, labels, metas) in enumerate(loader, start=1):
            ...
            true_probs = probs[torch.arange(labels.size(0), device=probs.device), labels].cpu().tolist()
            for meta, truth, pred_idx, confidence, true_prob in zip(metas, batch_true, batch_pred, batch_conf, true_probs):
                row = {
                ...
                for class_idx, class_name in enumerate(mapping.class_order):
                    row[f"prob_{class_name}"] = float(probs[len(prediction_rows) % args.bag_batch_size, class_idx].item())
